...
#define mal_emerg(...)             do{printf(__VA_ARGS__);printf("\r\n");}while(0)
#define mal_crit(...)              do{printf(__VA_ARGS__);printf("\r\n");}while(0)
#define mal_err(...)               do{printf(__VA_ARGS__);printf("\r\n");}while(0)
#define mal_warning(...)           do{printf(__VA_ARGS__);printf("\r\n");}while(0)
#define mal_info(...)              do{printf(__VA_ARGS__);printf("\r\n");}while(0)
#define mal_debug(...)             do{printf(__VA_ARGS__);printf("\r\n");}while(0)
...
static int mal_mc_init(iotx_mc_client_t *pClient, iotx_mqtt_param_t *pInitParams)
{
    ...
    if (HAL_AT_MQTT_Init(pInitParams) != 0) {/*配置sim800 一些参数  打开*/
...
/************************  Start_Gprs_TCP 建立GPRS-TCP连接 ******0422******************/
//return_value=1 fail, return_value=0 succ, 
int Start_Gprs_TCP(iotx_mqtt_param_t *mqtt_params)
{
	int return_value = 1;
	 
	return_value = HAL_AT_MQTT_GRPS_TCP_Init(mqtt_params);
	return return_value;
}
/************************  Public Interface ************************/
void *wrapper_mqtt_init(iotx_mqtt_param_t *mqtt_params)
{
...
#ifndef PLATFORM_HAS_DYNMEM
    int idx;
		printf("__PLATFORM_HAS_DYNMEM-support\r\n");
#endif
    ...
    if (NULL == pclient) {
		...
		printf("__PLATFORM_HAS_DYNMEM-not support:pclient = & g_iotx_mc_client[0].used = 1 \r\n");
#endif
    ...
    printf("__mal_mc_init(pclient, mqtt_params) \r\n");
    return pclient;
}
