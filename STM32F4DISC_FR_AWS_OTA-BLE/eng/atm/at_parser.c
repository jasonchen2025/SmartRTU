...
#ifdef AT_DEBUG_MODE
#define atpsr_err(...)               do{printf(__VA_ARGS__);printf("\r\n");}while(0)
#define atpsr_warning(...)           do{printf(__VA_ARGS__);printf("\r\n");}while(0)
#define atpsr_info(...)              do{printf(__VA_ARGS__);printf("\r\n");}while(0)
#define atpsr_debug(...)             do{printf(__VA_ARGS__);printf("\r\n");}while(0)
#else
#define atpsr_err(...)               do{printf(__VA_ARGS__);printf("\r\n");}while(0)
#define atpsr_warning(...)           do{printf(__VA_ARGS__);printf("\r\n");}while(0)
#define atpsr_info(...)              do{printf(__VA_ARGS__);printf("\r\n");}while(0)
                       ...
                       const atcmd_config_t *atcmdconfig)
{
    ...
    HAL_Delay(at._timeout);
		 strncpy(replybuf, AT_RX_BUF, bufsize);
...
int at_send_no_reply(const char *data, int datalen, bool delimiter)
{
    ...
    if ((ret = at_sendto_lower(at._pstuart, (void *)data,
		...
		printf("__**at_send<%s>end******\r\n",data);													 

    if (delimiter) {
			//printf("__at_send_delimiter<%s>-******\r\n",at._send_delimiter);
        if ((ret = at_sendto_lower(at._pstuart, (void *)at._send_delimiter,
                    strlen(at._send_delimiter), at._timeout, false)) != 0) {
            atpsr_err("uart send delimiter failed");
            HAL_MutexUnlock(at.at_uart_send_mutex);
