...
#ifdef AT_DEBUG_MODE
#define at_conn_hal_err(...)               do{printf(__VA_ARGS__);printf("\r\n");}while(0)
#define at_conn_hal_info(...)              do{printf(__VA_ARGS__);printf("\r\n");}while(0)
#define at_conn_hal_debug(...)             do{printf(__VA_ARGS__);printf("\r\n");}while(0)
#else
#define at_conn_hal_err(...)               do{printf(__VA_ARGS__);printf("\r\n");}while(0)
#define at_conn_hal_info(...)              do{printf(__VA_ARGS__);printf("\r\n");}while(0)
...
static int sim800_uart_init(void)
{
    ...
    memset(rsp, 0, SIM800_DEFAULT_RSP_LEN);
    /*set baudrate 115200*/
    sprintf(cmd,  "%s=%d", AT_CMD_BAUDRATE_SET, AT_UART_BAUDRATE);
    ...
    sprintf(cmd, "%s=%d,%d", AT_CMD_FLOW_CONTROL, 0, 0);
    at_send_wait_reply(cmd, strlen(cmd), true, NULL, 0, rsp, SIM800_DEFAULT_RSP_LEN, NULL);
    if (strstr(rsp, SIM800_AT_CMD_SUCCESS_RSP) == NULL) {
        at_conn_hal_err( "%s %d failed rsp %s\r\n", __func__, __LINE__, rsp);
        return -1;
...
static int sim800_gprs_ip_init(void)
{
    ...
    memset(cmd, 0, SIM800_DEFAULT_CMD_LEN);
    memset(rsp, 0, SIM800_DEFAULT_RSP_LEN);
    sprintf(cmd, "%s=%d", AT_CMD_MULTI_IP_CONNECTION, 1);
    ...
    memset(rsp, 0, SIM800_DEFAULT_RSP_LEN);
    memset(cmd, 0, SIM800_DEFAULT_CMD_LEN);
    sprintf(cmd, "%s=%d", AT_CMD_SEND_DATA_PROMPT_SET, 0);
    at_send_wait_reply(cmd, strlen(cmd), true, NULL, 0, 
                       rsp, SIM800_DEFAULT_RSP_LEN, NULL);
    if (strstr(rsp, SIM800_AT_CMD_SUCCESS_RSP) == NULL) {
        at_conn_hal_err( "%s %d failed rsp %s\r\n", __func__, __LINE__, rsp);
    ...
    memset(rsp, 0, SIM800_DEFAULT_RSP_LEN);
    memset(cmd, 0, SIM800_DEFAULT_CMD_LEN);
    sprintf(cmd,"%s=%d", AT_CMD_RECV_DATA_FORMAT_SET, 1);
    at_send_wait_reply(cmd, strlen(cmd), true, NULL, 0, rsp, SIM800_DEFAULT_RSP_LEN, NULL);
    if (strstr(rsp, SIM800_AT_CMD_SUCCESS_RSP) == NULL) {
        at_conn_hal_err( "%s %d failed rsp %s\r\n", __func__, __LINE__, rsp);
        return -1;
...
int HAL_AT_CONN_DomainToIp(char *domain, char ip[16])
{
    ...
    sprintf(pccmd, "%s=%s", AT_CMD_DOMAIN_TO_IP, domain);
...
//    switch (conn->type) {
//        case TCP_CLIENT:
//            sprintf(pccmd, SIM800_CONN_CMD_LEN - 1, "%s=%d,\"TCP\",\"%s\",%d", AT_CMD_START_CLIENT_CONN, linkid, conn->addr,
//                     conn->r_port);
...
//    memset(cmd, 0, SIM800_DEFAULT_CMD_LEN);
//    memset(rsp, 0, SIM800_DEFAULT_RSP_LEN);
//    sprintf(cmd, SIM800_DEFAULT_CMD_LEN - 1, "%s=%d", AT_CMD_STOP_CONN, linkid);
//    at_send_wait_reply(cmd, strlen(cmd), true, NULL, 0, 
//                       rsp, SIM800_DEFAULT_RSP_LEN, NULL);
//    if (strstr(rsp, SIM800_AT_CMD_SUCCESS_RSP) == NULL) {
                 ...
                 int32_t timeout)
{
    ...
    memset(cmd, 0, SIM800_DEFAULT_CMD_LEN);
    sprintf(cmd,  "%s=%d,%d", AT_CMD_SEND_DATA, linkid, len);
