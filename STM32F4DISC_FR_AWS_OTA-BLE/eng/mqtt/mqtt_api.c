...
void *IOT_MQTT_Construct(iotx_mqtt_param_t *pInitParams)
{
    ...
    do {
...
#ifdef SUPPORT_TLS
        {
            ...
            mqtt_params->pub_key = iotx_ca_crt;
					  printf("__Initialize MQTT parameter£ºSUPPORT_TLS\r\n"); //20190420print
        }
#endif
...
#endif
     //20190420print  --** Ö´ÐÐGPRSÁ¬½Ó Í¨¹ý +SAPBR: 11,"10.180.55.195"
		//<AT+SAPBR=3,1,"Contype","GPRS"¡£,<AT+SAPBR=3,1,"APN","CMNET",AT+SAPBR=1,1,;AT+SAPBR=2,1²éÑ¯£¬+SAPBR: 1,1,"10.10.255.195 
		//<AT+CPIN?£¬ +CPIN: READY £¬ <AT+CSQ £¬+CSQ: 29,0£¬ <AT+CREG?£¬+CREG: 0,1 ×¢²á³É¹¦£
		//<AT+CGATT £¬¬+CGATT: 1 GPRS attached³É¹¦£¬  <AT+SAPBR=2,1 ,, +SAPBR: 11,"10.180.55.195"
    pclient = wrapper_mqtt_init(pInitParams);/*×îÖ÷ÒªÊÇ¶Ôsim800½øÐÐÒ»Ð©µ×²ãµÄatÃüÁîÉèÖÃ step1-1*/
    if (pclient == NULL) {
        if (mqtt_params != NULL) {
            _iotx_mqtt_free_param(mqtt_params);
    ...
    //½¨Á¢GPRS-TCPÁ¬½Ó -Start_Gprs_TCP() 20190422   step1-2 
		 printf("__@@@@GPRS-TCP¿ªÊ¼Á¬½Ó****\r\n"); //20190422print
		if(Start_Gprs_TCP(pInitParams))        //½¨Á¢TCPÁ¬½Ó	
		{ 
			printf("__@@@@GPRS-TCPÁ¬½ÓÊ§°Ü¦****\r\n"); //20190422print 
		} 
		else 
		  printf("__@@@@GPRS-TCPÁ¬½Ó³É¹¦****\r\n"); //20190422print
